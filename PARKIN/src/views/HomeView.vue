<template>
  <div class="space-y-5">
    ...
    <div class="card" v-if="cctvUrl">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-bold" style="color: var(--surface-text);">Live Monitoring</h2>
        <span class="badge bg-status-danger/10 text-status-danger animate-pulse">
          <font-awesome-icon :icon="['fas', 'circle']" class="text-[8px] mr-1" /> LIVE
        </span>
      </div>
      <div class="relative w-full overflow-hidden rounded-xl bg-black" style="padding-top: 56.25%;">
        <!-- YouTube Embed -->
        <iframe 
          v-if="streamType === 'youtube' && youtubeId"
        ...
        <div v-else class="absolute top-0 left-0 w-full h-full flex items-center justify-center text-sm" style="color: var(--surface-muted);">
          Stream tidak tersedia atau bukan link YouTube.
        </div>
      </div>
    </div>
...
const cctvUrl = ref('')
const streamType = ref('youtube')

const youtubeId = computed(() => {
  if (!cctvUrl.value) return null
  const match = cctvUrl.value.match(/(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))([\w-]{11})/)
...
async function loadSettings() {
  try {
    ...
    if (res.data) {
      cctvUrl.value = res.data.stream_url || ''
      streamType.value = res.data.stream_type || 'youtube'
    }
  } catch (error) {
...
onMounted(async () => {
  ...
  unsubscribe = parkingStore.onVehicleUpdate((eventType, data) => {
    if (eventType === 'new_log' || eventType === 'stats') {
      ...
      cctvUrl.value = data.stream_url || ''
      streamType.value = data.stream_type || 'youtube'
    }
  })
})
