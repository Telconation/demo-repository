  ...
  <div class="space-y-5">
    <div class="card">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          <h2 class="text-xl font-bold" style="color: var(--surface-text);">Vehicle Logs</h2>
          <p class="text-xs mt-0.5" style="color: var(--surface-muted);">Filter dan lihat riwayat kendaraan</p>
        </div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 mb-5 p-4 rounded-xl" style="background: var(--surface-bg); border: 1px solid var(--surface-border);">
        <div class="flex-1">
          <label class="block text-xs font-semibold mb-1 uppercase tracking-wider" style="color: var(--surface-muted);">Tanggal Mulai</label>
          <input v-model="filters.start_date" type="date" class="input text-xs" />
        </div>
        <div class="flex-1">
          <label class="block text-xs font-semibold mb-1 uppercase tracking-wider" style="color: var(--surface-muted);">Tanggal Akhir</label>
      ...
      <div class="overflow-x-auto rounded-xl" style="border: 1px solid var(--surface-border);">
        <table class="table-auto">
          ...
          <tbody>
            <tr v-if="loading">
              <td colspan="5" class="text-center py-8" style="color: var(--surface-muted);">
            ...
            <tr v-else v-for="log in logs" :key="log.id">
              ...
              <td>
                <span class="capitalize text-sm">{{ log.jenis_kendaraan }}</span>
              </td>
              <td>
...
function buildParams(overrides = {}) {
  ...
  if (filters.jenis_kendaraan) params.set('vehicle_type', filters.jenis_kendaraan)
  if (filters.status) params.set('status', filters.status)
  if (filters.start_date) params.set('start_date', new Date(filters.start_date).getTime())
  if (filters.end_date) {
...
async function exportToCSV() {
  ...
  try {
    ...
    // Build CSV
    const headers = ['ID', 'Jenis Kendaraan', 'Status', 'Track ID', 'Confidence', 'Waktu']
