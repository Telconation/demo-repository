        ...
        <table class="table-auto">
          ...
          <tbody>
            ...
            <tr v-else-if="errorMsg">
              <td :colspan="isAdmin ? 7 : 6" class="text-center py-8">
                <span class="text-status-danger text-sm">{{ errorMsg }}</span>
              </td>
            </tr>
...
async function loadUsers() {
  ...
  try {
    ...
    errorMsg.value = error.message || 'Gagal memuat data pengguna'
  } finally {
    loading.value = false
  }
}
...
function openEditForm(user) {
  if (user.id === currentUserId.value) {
    alert('Gunakan menu Profil Saya untuk mengedit data Anda sendiri.');
    return;
  }
  editMode.value = true
